...
export default function Onboarding() {
  ...
  // index 1-7 used; index 0 ignored
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // If they already have >= 2 photos, jump at least to Profile (step 4)
        if (photoCount >= 2 && initialStep <= 3) {
          initialStep = 4;
        }

        // If step 4 is already completed (and photos requirement satisfied),
        // jump to Details (step 5)
        if (hasBasicProfile && photoCount >= 2 && initialStep <= 4) {
          initialStep = 5;
        }

        // If personal details (step 5) are completed, jump to Islamic (step 6)
        if (
          hasPersonalDetails &&
          photoCount >= 2 &&
          hasBasicProfile &&
        ...
        // If Islamic preferences (step 6) are already filled,
        // and previous steps satisfied, jump to Verify (step 7)
        if (
        ...
        const initialCompleted = Array(8).fill(false) as boolean[];
        ...
        initialCompleted[4] = hasBasicProfile && initialCompleted[3];
        initialCompleted[5] = hasPersonalDetails && initialCompleted[4];
        initialCompleted[6] = hasIslamicPreferences && initialCompleted[5];
        // Step 7 completion is effectively when onboarding is done, which happens
        // when they leave this page, so we don't set it here.
      ...
      <div className="max-w-4xl mx-auto">
        ...
        {step === 2 && (
          <WaliInvitation
            ...
            existingWali={
              waliInfo
                ? { email: waliInfo.email, phone: waliInfo.phone }
                : null
            }
          />
        )}
