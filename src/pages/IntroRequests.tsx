...
export default function IntroRequests() {
  ...
  try {
    ...
    // 1) Fetch the actual intro request row so we know from/to/wali
    const { data: reqRow, error: reqFetchError } = await supabase
      .from('intro_requests')
      .select('id, from_user_id, to_user_id, wali_id, status, wali_approved')
      .eq('id', id)
      .single();

    if (reqFetchError || !reqRow) {
      console.error('Error fetching intro_request:', reqFetchError);
      toast.error('Could not load this introduction. Please try again.');
      return;
    }

    const isCurrentUserWali = reqRow.wali_id === user.id;
    ...
    if (nextStatus === 'rejected') toast('You rejected this introduction.');

    // 3) Send emails (best-effort — don’t block UI)
    try {
      const appUrl = window.location.origin;

      // Load both profiles for names + emails
      ...
      const fromName =
        `${fromP?.first_name ?? ''} ${fromP?.last_name ?? ''}`.trim() || 'Someone';
      const toName =
        `${toP?.first_name ?? ''} ${toP?.last_name ?? ''}`.trim() || 'Someone';

      // If approved -> treat as "match" notification to both (go to /messages)
      if (nextStatus === 'approved') {
        const res1 = fromP?.email
          ? await supabase.functions.invoke('send-notification-email', {
              body: {
                type: 'match',
                to: fromP.email,
                recipientUserId: fromP.id,
                data: {
                  matchName: toName,
        ...
        if (res1?.error) {
          console.error('send-notification-email (fromP) invoke error:', res1.error);
        } else {
          console.log('send-notification-email (fromP) ok:', res1?.data);
        }

        const res2 = toP?.email
          ? await supabase.functions.invoke('send-notification-email', {
              body: {
                type: 'match',
                to: toP.email,
                recipientUserId: toP.id,
                data: {
                  matchName: fromName,
                  loginUrl: `${appUrl}/messages`,
                },
        ...
        if (res2?.error) {
          console.error('send-notification-email (toP) invoke error:', res2.error);
        } else {
          console.log('send-notification-email (toP) ok:', res2?.data);
        }
      }

      // If rejected -> notify the sender (go to /intro-requests)
      if (nextStatus === 'rejected') {
        const rejectedBy = isCurrentUserWali ? 'Your wali' : 'The recipient';

        const res = fromP?.email
          ? await supabase.functions.invoke('send-notification-email', {
              body: {
                type: 'intro_request',
                to: fromP.email,
                ...
                data: {
                  ...
                  message: `${rejectedBy} has rejected the introduction request.`,
        ...
        if (res?.error) {
          console.error('send-notification-email (rejected notify) invoke error:', res.error);
        } else {
          console.log('send-notification-email (rejected notify) ok:', res?.data);
        }
      }
    } catch (emailErr) {
      console.error('Failed to send intro status emails:', emailErr);
      // don’t show error toast; status update already succeeded
    }
  } catch (err) {
