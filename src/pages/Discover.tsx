...
export default function Discover() {
  ...
  // ─────────────────────────────
  // Intro Request handler (used by star button / Super-Intro)
  // ─────────────────────────────
    ...
    try {
      ...
      if (!canUse) {
        ...
        if (!profile || profile.subscription_status !== 'active' || tier === 'free') {
          ...
          // Gold should be unlimited, but just in case
          toast.error(
      ...
      // 3) Insert intro_request; wali_id will be set in DB trigger
      ...
      // 4) Move to next profile locally
  ...
  const handleSwipe = async (direction: SwipeDirection) => {
    ...
    // For the star button: send intro request instead of regular like (Super-Intro)
    if (direction === 'up') {
      await handleIntroRequest(currentProfile);
      // handleIntroRequest already increments index when successful
      return;
    }
    ...
    try {
      ...
      if (action === 'like') {
        ...
        if (!canLike) {
          ...
          if (!profile || profile.subscription_status !== 'active' || tier === 'free') {
            ...
            // Silver/Gold should be unlimited, this is a safety fallback
            toast.error(
              'You have reached your daily like limit. Please try again tomorrow.'
            );
          }

          return; // do not save like or advance to next profile
        }
      ...
      // Only advance when we actually processed the swipe
      setCurrentIndex((i) => i + 1);
    }
  };
...
//       // 2) Insert intro_request; wali_id will be set in DB trigger
//       const { error: insertError } = await supabase.from('intro_requests').insert({
//         from_user_id: user.id,
//         to_user_id: target.id,
//         message: null, // later we can add a custom message
...
//       // 3) Move to next profile locally
//       setCurrentIndex((i) => i + 1);
//     } catch (err) {
//       console.error('Intro request error:', err);
//       toast.error('Something went wrong while sending your request.');
...
//     if (direction === 'up') {
//       await handleIntroRequest(currentProfile);
//       // handleIntroRequest already increments index
//       return;
//     }
