...
export const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose }) => {
  const [mode, setMode] = useState<'auth' | 'check' | 'success'>('auth');
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
      await signUp(email, password); // sends the confirmation link
      setIsNewUser(true);
      setMode('check'); // show "check your inbox"
    } catch (err: any) {
          ...
          <DialogHeader>
            <DialogTitle className="text-2xl font-bold text-teal-800">
              {mode === 'auth' ? 'Welcome to Nikah Connect' :
               mode === 'check' ? 'Verify Your Email' : 'Success!'}
            </DialogTitle>
          </DialogHeader>
            ...
            <div className="space-y-4">
              <Alert className="border-blue-200 bg-blue-50">
                ...
                <AlertDescription className="text-blue-700">
                  We sent a confirmation link to <strong>{email}</strong>. Click it to verify and finish signup.
                </AlertDescription>
              </Alert>
            ...
            <div className="space-y-4 py-4">
              ...
              <div className="text-center space-y-2">
                ...
                <p className="text-sm text-gray-600">
                  {isNewUser
                    ? 'Your email is verified. Redirecting to complete your profile...'
